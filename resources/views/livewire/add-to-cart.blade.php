<div>
  <div class="w-full h-[512px] bg-white absolute bottom-0 left-0 rounded-t-2xl overflow-scroll" x-data="{ open: @entangle('showOverlay') }"
    ...
    <button
      class="text-2xl absolute top-0 right-0 mr-4 mt-4 rounded-full px-2 pb-1 inline-flex justify-center items-center bg-black text-white"
    ...
    <div class="mt-4 mx-4">
      ...
      <div class="flex justify-center items-center">
        ...
        <input class="w-10 text-xs text-center border-gray-400 rounded" type="text" value="{{ $quantity }}"
          wire:model="quantity" />
        <button class="text-xl ml-4" type="button" wire:click.defer="$emit('increment', 'getSubtotal')">+</button>
      </div>
    </div>
