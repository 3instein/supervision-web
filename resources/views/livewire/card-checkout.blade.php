  ...
  <li class="flex py-6 sm:py-10">
    <div class="flex-shrink-0">
      <img src="{{ $menu->image }}" alt="{{ $menu->name }}"
        class="w-32 h-32 rounded-md object-center object-cover sm:w-48 sm:h-48">
    </div>
      ...
      <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
        <div>
          <div class="flex justify-between">
            <h3 class="text-sm">
              <label class="font-bold text-gray-700 hover:text-gray-900">
                {{ $menu->name }}
              </label>
            </h3>
          </div>
          <p class="mt-1 text-sm font-medium text-gray-700">Rp. {{ number_format($menu->price ?? 0) }}</p>
        </div>

        <div class="mt-4 sm:mt-0 sm:pr-9 mb-4">
        ...
        @if ($note)
          <div class="mt-4 sm:mt-0 sm:pr-9">
            <p class="text-sm text-gray-700">Catatan: {{ $note }}</p>
          </div>
        @endif
      </div>
    </div>
